package perl_api::Controller::API;
use Moose;
...
sub auto : Private {
    ...
    if ( ! defined $' ) {
        $c->res->body( $c->encoding->encode( '{"RTN_MSG": "访问接口不存在", "RTN_MSG": "-1"}', $c->_encode_check ) );
        $c->detach();
    }
    my $db_key = $';
    ...
    if ( scalar keys %{ $c->req->parameters }  < scalar @uniq_array ) {
        $c->{ RTN_CODE } = "04";
        $c->{ RTN_MSG } = "参数不完整";
        $c->detach("error");
    }
    elsif ( scalar keys %{ $c->req->parameters }  > scalar @uniq_array ) {
        $c->{ RTN_CODE } = "04";
        $c->{ RTN_MSG } = "参数过多";
        $c->detach("error");
    }
    ...
    foreach my $req_key ( keys %{ $c->req->parameters } ) {      # 允许的字段数组
